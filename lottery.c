...
void SpecifyTheDrawNumber(){
	...
	if(canBuy!=1){
		...
		printf("\n\t本期开奖号：");
		printf("%02d-%02d-%02d-%02d-%02d-%02d---%02d",red[0],red[1],red[2],red[3],red[4],red[5],blue);
		UpdateUserBalancesAfterDraw(canBuy,drawNum);	//开奖后更新用户余额
		ModifyLotteryState(canBuy);			//修改彩票开奖状态为1
		UpdateDrawNumberAfterTheDraw(canBuy,drawNum);	//开奖后更新开奖号
	}else{
...
void displayLotteryPurchaseHistory(char account[100]){
	printf("\n\t彩票期号  单注金额         用户开奖号         购买注数  购买金额         UID         中奖状态  中奖金额\n\n");
	UserBuy *temp=UserBuyhead;
	while(temp!=NULL){
		if(strcmp(temp->data->account,account) == 0){
			int lotterBets=findLotteryBets(temp->data->number);
			int spend=lotterBets*temp->data->bets;
			printf("\t%d     %d       %02d-%02d-%02d-%02d-%02d-%02d---%02d\t  %d\t  %d\t    %ld\t%d\t %.2f\n",temp->data->number,lotterBets,temp->data->drawNumber[0],temp->data->drawNumber[1],temp->data->drawNumber[2],temp->data->drawNumber[3],temp->data->drawNumber[4],temp->data->drawNumber[5],temp->data->drawNumber[6],temp->data->bets,spend,temp->data->uid,temp->data->winStatus,temp->data->winAmount);
...
void findLotterInfo(){			//查询某一期彩票中奖信息
	...
	scanf("%d",&number);
	UserBuy *temp=UserBuyhead;
	printf("\n\t彩票期号  单注金额         用户开奖号         购买注数  购买金额         UID         中奖用户  中奖金额\n\n");
	while(temp!=NULL){
		if(temp->data->number == number && temp->data->winStatus==1){
			int lotterBets=findLotteryBets(temp->data->number);
			int spend=lotterBets*temp->data->bets;
			printf("\t%d     %d       %02d-%02d-%02d-%02d-%02d-%02d---%02d\t  %d\t  %d\t    %ld       %s\t%.2f\n",temp->data->number,lotterBets,temp->data->drawNumber[0],temp->data->drawNumber[1],temp->data->drawNumber[2],temp->data->drawNumber[3],temp->data->drawNumber[4],temp->data->drawNumber[5],temp->data->drawNumber[6],temp->data->bets,spend,temp->data->uid,temp->data->account,temp->data->winAmount);
		}
		temp=temp->next;
	}
}
